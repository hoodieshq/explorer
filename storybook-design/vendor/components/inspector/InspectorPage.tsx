...
import { MIN_MESSAGE_LENGTH, RawInput } from '@/app/components/inspector/RawInputCard';
...
import { SimulatorCard } from '../../features/instruction-simulation/ui/SimulationCard';
...
export function TransactionInspectorPage({
    ...
    // Recolour every dashkit card outline on the inspector page to outer-space-800. Scoped here
    // (not in BaseCard/CollapsibleSection, both shared with the tx-details page) via the shared
    // `border-dk-card-outline-dark` class hook so section cards, instruction cards, ATL and CU
    // profiling all pick it up without bleeding onto other pages.
    return (
...
function LoadedView({
    ...
    // Block spacing mirrors the transaction details page (tx/[signature]/page-client.tsx).
    return (
