/* eslint-disable unicorn/no-null -- forked from ./AccountsCard (itself a vendored copy of
   app/components/inspector/AccountsCard.tsx) for the Match-to-TX-view merged table; keeps the same
   null-returning render style as the upstream. */
// Match-to-TX-view-only Account List. It merges the standalone "SOL Balance Changes" table into the
// Account List as a "Change" column placed right before "Post Balance". The change values come from
// the simulation:
//   • before a run — the first row shows a small Simulate button in place of the value; every other
//     row shows a grey dash that turns into a Simulate button on row hover;
//   • after a run — each account shows its SOL balance delta (a grey dash if it did not change).
// This is a private fork so the shared ./AccountsCard (used by Default/Enhancements) is untouched.
...
import { LastSimulatedAtLabel, useLastSimulatedAt } from '../../features/instruction-simulation/ui/LastSimulatedAt';
import { Section } from '../../shared/ui/Section';
import { AccountDetailSlideover } from './AccountDetailSlideover.match-to-tx-view';
...
// Section card chrome, gated to lg+. On mobile each account entry renders as its own card
// (AccountRowLayout), so the Section wrapper must NOT add its own card surface there — otherwise the
// per-row cards sit inside a second card (card-in-card). At lg+ the rows form a table, so the dashkit
// card surface is applied to frame it. Mirrors baseCardVariants({ ui: 'dashkit' }) at the lg breakpoint.
const LG_ONLY_CARD = cn(
...
export function AccountsCard({ message, simulation }: { message: VersionedMessage; simulation: SimulationState }) {
    ...
    // Tracked here (a persistently-mounted host) so the header popover can show the last run's time
    // even though its own content mounts only while open.
...
// row's text line-height — so the button never grows the row taller than the plain dash/delta state
// (i.e. the row height doesn't jump between pre-run, hover and post-run). `table` is the tiny in-cell
// form; `drawer` keeps the same height with slightly larger, more legible text for the detail popup.
...
// "Simulated at …" line once a run has completed. It is a popover (not a tooltip) because the content
// is interactive; opening is driven by hover with a short close delay so the pointer can travel from
// the badge into the popover (which portals out of the header) without it snapping shut.
...
//     carries `group`; gated to mouse devices via `@media (hover: hover) and (pointer: fine)` so on
//     touch, where hover is emulated and would stick, the dash simply stays).
    ...
    return (
        ...
        <div className="lg:border-1 group lg:border-b lg:border-white/10 lg:[border-bottom-style:solid] lg:last:border-b-0">
            {/* Mobile layout: a tappable card (address + badges, ordinal, change, balance). The card
                framing makes it clear the whole entry is tappable. Owner and size drop out here and
                move into the detail drawer, mirroring the TX page. Tapping anywhere opens the drawer. */}
            <div
                className={cn(
            ...
            {/* Mobile detail drawer (portaled; only reachable via the mobile summary's tap). Carries the
                same fields as the list — Owner, Change, Balance, Size — with the Simulate button living
                in its Change row (`mode="action"`). */}
            {pubkey && (
...
// The interactive size element: shows the account's data size and, on click, opens the raw-data
// viewer (Hex / Base64 with copy and download) in a popover — the shared RawDataField used on the
...
function AccountDataSize({
    ...
    return (
        <Popover>
            ...
            {/* RawDataField brings its own card chrome, so the popover wrapper is left transparent
                (its drop shadow is kept for depth). */}
