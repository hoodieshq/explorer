...
import { CheckCircle, ChevronDown, Copy, Download } from 'react-feather';
...
export type RawDataFieldProps = {
     ...
     *                matching the `popover` "show more" behaviour.
     */
    variant?: 'popover' | 'embedded';
    /**
     * `popover` only: render the copy/download actions icon-only (drops their
     * `md:inline` text labels). Useful when the popover is embedded in a narrow
     * host (e.g. the mobile drawer) where the labels would crowd the header.
     */
...
export function RawDataField({
    ...
    // clamp is revealed inline via an Expand/Collapse toggle ("Show more"), like
    // the popover's "show more". Expanding drops the max-height clamp so the full
    // payload flows inline.
    if (variant === 'embedded') {
        ...
        return (
            <Tabs value={tab} onValueChange={handleTabChange} className="w-full overflow-hidden">
                ...
                {/* Collapsed: show at most 4 rows; the rest is revealed inline via the
                    ...
                    padding (0.75rem) + 4×1rem line-height. Expanded drops the clamp so
                    the full payload flows inline. */}
        ...
        >
            <div
                className={cn(
                    ...
                    // byte count + buttons on one line — no wrap, tighter gap. The default
                    // (desktop popover) keeps the roomy wrapping header.
                    iconOnlyActions ? 'flex-nowrap items-center gap-3' : 'flex-wrap gap-8',
                )}
            >
                <TabsList>
                    <TabsTrigger className="!py-2 text-sm" value="hex">
                        Hex
                    </TabsTrigger>
                    <TabsTrigger className="!py-2 text-sm" value="base64">
                        Base64
                    </TabsTrigger>
                </TabsList>
            ...
            >
                {loading ? (
                    ...
                    <span className="text-wrap break-all font-mono text-sm text-white">
                        {visibleBase64}
