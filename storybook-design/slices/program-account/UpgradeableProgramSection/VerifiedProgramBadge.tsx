 ...
 * soft badge (the pre-existing content), rebuilt on the tw `Badge` instead of dashkit: `tone="soft"`
 * reuses the same success/warning hues, and `whitespace-normal` lets the label wrap in a narrow
 * column (tailwind-merge overrides the badge's base `whitespace-nowrap`, so no `!important`).
 */
export function VerifiedBuildBadge({ state, size = 'xs' }: { state: VerifiedBuildState; size?: 'xs' | 'sm' }) {
    if (state === 'loading') {
        // Skeleton stands in for the value text; the row height is held by the label's
        // line-box, and align-middle centers the bar on the text line so nothing shifts.
        return (
            <span className="min-w-0 break-words">
                <Skeleton className="relative -top-0.5 inline-block h-3.5 w-40 align-middle" />
            </span>
        );
    }
...
export function VerifiedProgramBadge({
    ...
    // Clean design-system (tw) badge for every state — the presentational VerifiedBuildBadge
    // already renders each state (incl. the loading skeleton), so no dashkit variant is needed.
