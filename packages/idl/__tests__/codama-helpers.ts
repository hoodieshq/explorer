...
export function fetchedJson(document: CodamaIdlInput): unknown {
    return JSON.parse(JSON.stringify(document));
}

/** Build the named zero-argument instruction with codama's OWN dynamic client (every account defaulted). */
export async function buildInstruction(document: CodamaIdlInput, name: string): Promise<Instruction> {
...
export function encodeAccount(document: CodamaIdlInput, name: string, data: object): Uint8Array {
    const root = document as unknown as CodamaIdl;
