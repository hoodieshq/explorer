...
const buildInspectEntityDescription = (clusterNames: EnabledClusterNames) =>
    [
        ...
        '- "bpf-upgradeable-loader": Upgradeable programs — address, label, balance, executable-data account, upgradeability, last deploy slot, upgrade authority, plus "idl" (status, idl_type, source, program name), "verification" (verified-build status), "security_metadata" (security.txt) and "multisig" (upgrade-authority multisig) enrichments. Each carries an explicit unknown marker when its source is unavailable.',
        '- "bpf-loader" / "bpf-loader-2" / "loader-v4": Legacy-loader programs — currently unsupported; return a CURRENTLY_UNSUPPORTED error.',
        '- "unknown": Unrecognized account type. When the owner program publishes an IDL, the account data is decoded through it and returned as "decoded" (source "idl"). "decoded.layout" reports where each decoded value sits in the raw bytes: a dot "path" that reads that value out of "decoded.info", the byte "offset" and "size", the schema "kind" (plus "format" for numbers), and the program\'s own "docs" when it declares any. Rows cover named fields plus the container bodies and the container elements they nest in — an element that is not a container stays a value on its array, so a "[u8; 32]" field is one row — and every "path" appears exactly once. A range covers the codec framing the schema does not name — a length prefix, an option tag, an enum discriminant — so it is the field\'s extent, not its payload\'s. Where the framed member has a row of its own, that framing is the gap between a parent\'s range and its children\'s: a vec starting at 8 whose first element starts at 12 has its 4-byte count in between. A long layout is capped and reports "omitted".',
        '',
