...
The added cost is bounded and lands only where it is needed. One bulk lookup for 1051 mints is a single request — 35 KB up, 17 KB down gzipped, 0.42 s. The page already downloads 85 KB gzipped of token accounts before any of this, so the sort adds about a fifth to a transfer that already happens, and it replaces the ~53 lazy round trips a fully expanded list costs today.
...
- Send the bulk lookup through `POST /api/token-info` rather than the `'use server'` action `TokenInfoBatchProvider` calls. The route carries `CACHE_MAX_AGE` and CDN headers, so repeat views of the same address share the result; a server action bypasses the CDN.
- Raise `MAX_ADDRESSES` in `app/api/token-info/config.ts` and correct its comment, which still claims `TokensProvider` sends at most 101 token accounts — untrue since #1181. The cap stays the chunk size the client splits on, so it bounds one request without bounding the list.
...
- **Latency:** the card's first paint waits on the bulk lookup, measured at 0.42 s for 1051 mints. Reordering after paint was rejected as visually worse.
