...
Metadata for the holdings list SHALL be requested through `POST /api/token-info`, which carries the Next data cache and CDN headers, and MUST NOT use the `'use server'` action that per-row lookups call, because a server action bypasses the CDN and makes every visitor re-pay the upstream round trip. The route's `MAX_ADDRESSES` cap SHALL be raised above its current 128 and SHALL define the chunk size the client splits on.
