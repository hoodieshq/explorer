...
- **Migrate all `*DetailsCard` components to consume `SliceParsed` now.** Deferred. A transitional compat wrap (`toParsedInstruction`) keeps the ~15-20 cards and their superstruct validators unchanged. Collapsing the render pipeline and deleting the wrap is Phase 5.
...
This proposal covers **Phase 1** (foundation + compat-layer architecture). Phases 2-5 are described but not yet implemented.
...
- The capability spec lives at `openspec/specs/instruction-parser/spec.md` (optional artifact, included here because the contract is non-trivial).
...
- **Accepted transitional debt.** `toParsedInstruction` / `toParsedTransaction` are the *only* permitted shims and live in one file (`model/compat.ts`); double validation occurs while tx-page input flows through both `slice.fromParsed` and the card's `create(...)`. Phase 5 (cards consume `SliceParsed` directly) deletes the wrap and the second pass in one go.
...
- **Known follow-ups:** Phase 1 is net-positive on its own (drift is mechanically prevented) but the "one render pipeline" goal needs Phase 5. Token-2022 `Initialize*Pointer` validators still expect non-nullable PublicKeys where the program allows `None` (pre-existing; parser passes `undefined`). Inspector currently decodes outer instructions only.
