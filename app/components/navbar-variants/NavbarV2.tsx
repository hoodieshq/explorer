...
export function NavbarV2({ children }: INavbarProps) {
        ...
        <>
            <nav className="flex flex-wrap items-center border-0 border-b border-solid border-outer-space-800 bg-heavy-metal-850 py-3 text-white">
                ...
                >
                    ...
                    {/* Brand lockup: Solana mark over an "Explorer beta" caption. Below lg it is a flow item that
                    takes the space between the burger and the cluster button and centres its own content, so
                    the lockup sits equidistant from the two — viewport-centring put it off-centre between
                    them, the buttons being different widths. Static and left-aligned from lg up. Kept short
                    (mark 18px + caption) so it never exceeds the cluster button, leaving the bar height
                    unchanged. */}
                    <Link
                        href={homePath}
                        className="flex min-w-0 flex-1 flex-col items-center leading-none lg:flex-none lg:items-start"
                    >
                        ...
                        <span className="mt-0.5 text-[9px] font-medium uppercase tracking-[0.12em] text-[#b4b4b4] lg:ml-[8px]">
                            Explorer (beta)
                        </span>
                    </Link>
                    ...
                    <div className="hidden h-full flex-1 items-center pl-6 pr-2 lg:block" style={{ minWidth: 0 }}>
                        {children}
                    </div>
                    ...
                    <div className="ml-[3px] block max-w-[210px] shrink-0 [&>*]:!inline-flex [&>*]:!h-[38px] [&>*]:!items-center [&>*]:!justify-center [&>*]:!py-0">
                        <ClusterStatusButton />
                    </div>
                </div>
            </nav>

            {/* Below lg the search sits under the bar instead of inside it.
            Owned by the variant, not the layout: this bar and the left-lockup one move the field into the bar at
            ...
            <div className="my-3 w-full px-4 lg:hidden">{children}</div>
        </>
    );
}
