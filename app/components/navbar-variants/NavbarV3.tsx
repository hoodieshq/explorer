...
import { ClusterStatusStacked } from './ClusterStatusStacked';
import type { INavbarProps } from './types';

export function NavbarV3({ children }: INavbarProps) {
        ...
        <>
            <nav className="flex flex-wrap items-center border-0 border-b border-solid border-outer-space-800 bg-heavy-metal-850 py-3 text-white">
                ...
                >
                    ...
                    is the rightmost flow item below lg, mirroring v1 where it was DOM-first on the left. */}
                    <div className="ml-[3px] flex min-w-0 items-center gap-2">
                        ...
                        {/* Nothing forced from out here, unlike v1 and v2, where the shipping cluster button has to
                            be pinned to the search field's 38px from its wrapper. This control is the
                            variant's own component and sets its own height and its own fixed width, so the
                            wrapper has no cap to apply. */}
                        <div className="block shrink-0">
                            <ClusterStatusStacked />
                        </div>
                        {/* The same outline as the cluster control beside it, by carrying the same classes
                            rather than overriding a filled variant: transparent ground, `outer-space-700`
                            border, matching radius and hover. A dashkit `Button` would need `!important` to
                            beat its own variant's background, and the two frames would drift the moment
                            either palette moved.

                            The icon is 18px, not the 24px of variant 1, so the glyph clears the 38px the
                            control and the search field share. */}
                        <button
                            type="button"
                            aria-label="Toggle navigation"
                            onClick={navHandlers.toggle}
                            className="flex h-[38px] shrink-0 cursor-pointer items-center justify-center rounded-md border border-solid border-outer-space-700 bg-transparent px-2 text-white transition-colors hover:border-outer-space-600 lg:hidden"
                        >
            ...
            Owned by the variant, not the layout: v1 and v2 move the field into the bar at
