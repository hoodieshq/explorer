import { privateConvertType } from '../converters/convert-reference-legacy-idl';
...
describe('[idl] convert-reference-legacy-idl', () => {
    describe('convertType', () => {
        it('should parse leaves as described, without casting into supported type', () => {
            ...
            expect(privateConvertType(leavesTypeArg.type)).toEqual({
        ...
        it('should not parse type.option.tuple', () => {
            ...
            expect(() => privateConvertType(type)).toThrow(new Error('Unsupported type: {"tuple":["u64","u64"]}'));
        });

        it('should not parse type.vec.tuple', () => {
            const type = { vec: { tuple: ['string', 'string'] } };
            expect(() => privateConvertType(type)).toThrow(
                new Error('Unsupported type: {"tuple":["string","string"]}')
            );
        });
    });
});
