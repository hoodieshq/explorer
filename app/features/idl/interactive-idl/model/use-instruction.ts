...
export function useInstruction({
    ...
    const programId = useMemo(() => new PublicKey(pid), [pid]);
    ...
    const invokeInstruction = useCallback(
        async (instructionName: string, instruction: InstructionData, params): Promise<string | null> => {
