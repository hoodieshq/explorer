...
import { ArrowRight, Check, Copy, ExternalLink, Globe, Link2, RotateCcw, Shield, XCircle } from 'react-feather';
...
const SECTIONS = [
    { id: 'setup', kicker: 'Setup', num: '01' },
    { id: 'instructions', kicker: 'Instructions', num: '02' },
    { id: 'tools', kicker: 'Tools', num: '03' },
    { id: 'examples', kicker: 'Examples', num: '04' },
] as const;
...
export function McpDocsOverviewViewV32() {
    ...
    return (
        ...
        >
            ...
            <Benefits />
            <NumberedBand section={SECTIONS[0]}>
                <Setup origin={origin} />
            </NumberedBand>
...
function Hero({ origin, status }: { origin: string; status: EndpointStatus }) {
    ...
    // live-status line in the first column, so the ledger on the right is pared down to the
    // four facts that actually sell the thing: auth, which clusters, which tools, and safety.
    const host = origin.replace('https://', '').replace('http://', '');
    const facts = [
        { key: 'Auth', num: '01', value: 'Open — no key required' },
        { key: 'Clusters', num: '02', value: 'mainnet-beta · devnet · testnet · simd296' },
        { key: 'Tools', num: '03', value: 'inspect_entity · ping' },
        { key: 'Writes', num: '04', value: 'None. Nothing signs or sends.' },
    ];
    ...
    return (
        ...
        <div
            ref={fieldRef}
            className={cn('relative w-full overflow-hidden pb-11 pt-5 sm:pb-[72px] sm:pt-8', GUTTER, BAND_RULE)}
        >
            ...
            <div className="relative z-[1] flex w-full flex-col gap-9">
                ...
                */}
                ...
                <h1 className="m-0 mt-[72px] max-w-[1120px] text-[34px] font-normal leading-[37px] tracking-[-1.1px] text-[#EDF2F0] sm:mt-[108px] sm:text-[46px] sm:leading-[48px] sm:tracking-[-1.5px] lg:text-[58px] lg:leading-[60px] lg:tracking-[-1.9px] xxl:text-[72px] xxl:leading-[75px] xxl:tracking-[-2.4px]">
                    Your agent stops guessing about&nbsp;the&nbsp;chain
                </h1>
                ...
                <div className="-mt-6 flex w-full flex-col gap-[52px] pt-3 sm:gap-[60px] lg:mt-0 lg:flex-row lg:gap-16">
                    ...
                    <div className="flex w-full flex-col items-start gap-7 lg:flex-1">
                        <p className="m-0 text-[15px] leading-[25px] text-[#8B9B94] sm:text-[18px] sm:leading-[30px]">
                            Point any MCP client at the Explorer and your agent reads decoded on-chain state directly —
                            accounts, programs, tokens, transactions — with the same IDL decoding and enrichments the
                            Explorer itself renders. One line of setup, no key, nothing that can sign.
                        </p>
                        <GravityCta href="#setup" fieldRef={fieldRef} className="text-[15px]">
                    ...
                    <div className="flex w-full flex-col lg:w-[400px]">
                        ...
                        <div className="flex w-full items-stretch gap-2.5 border-0 border-b border-solid border-[#242E2B] pb-3.5">
                            <span
                                aria-hidden
                                className={cn(
                        ...
                        <div className="grid grid-cols-2 lg:flex lg:flex-col">
                            {facts.map((fact, index) => (
                                ...
                                >
                                    <div className="flex items-center gap-2 lg:w-[116px] lg:shrink-0">
                                        <span className="font-mono text-[10px] text-dark-accent">{fact.num}</span>
...
function StatusNote({ status }: { status: EndpointStatus }) {
    ...
    return <span className="text-[13px] text-dark-accent">Enabled</span>;
}
...
function GravityCta({
    ...
    useEffect(() => {
        ...
        // Touch has no hover: switch gravity on while the button's centre sits within the middle
        // quarter of the viewport, so scrolling it into the centre pulls the field in the way a
        // hover would on desktop. On hover-capable devices this stays off and the pointer drives it.
        ...
        const updateZone = () => {
            ...
            zoneRef.current = centerY >= vh * 0.375 && centerY <= vh * 0.625;
        };
        updateZone();
    ...
    return (
        ...
        >
            ...
            <a
                ...
                className={cn(
                    'relative flex items-center gap-2.5 rounded-full bg-dark-accent px-6 py-3.5',
...
const BENEFITS = [
    {
        // The design draws the JSON braces as a glyph, not an icon.
        icon: <span className="font-mono text-[15px] leading-[19px] text-dark-accent">{'{ }'}</span>,
        text: 'Mints and Token-2022 extensions, IDLs, verified builds, instruction traces — parsed the way the Explorer parses them.',
        title: 'Decoded, not raw bytes',
    },
    {
        icon: <Globe size={19} className="text-dark-accent" aria-hidden />,
        text: 'mainnet-beta, devnet, testnet and simd296, switched with a single argument.',
        title: 'Every cluster, one endpoint',
    },
    {
        icon: <Shield size={19} className="text-dark-accent" aria-hidden />,
        text: 'Nothing signs, sends or mutates. Safe to hand to an autonomous agent.',
...
function Benefits() {
    return (
        <div className={cn('flex w-full flex-col pb-[28px] pt-8 sm:flex-row sm:pb-[52px] sm:pt-11', GUTTER, BAND_RULE)}>
            {BENEFITS.map((benefit, index) => (
                <div
                    key={benefit.title}
                    className={cn(
                        'flex flex-col gap-[13px] sm:flex-1',
                        // No dividers between factoids — just spacing that turns with the axis;
                        // half the column gap on tablet, full from `lg`.
                        index > 0 && 'mt-6 pt-6 sm:mt-0 sm:px-4 sm:pt-0 lg:px-8',
                        index === 0 && 'sm:pr-4 lg:pr-8',
                    )}
                >
                    <span className="flex h-[19px] items-center">{benefit.icon}</span>
                    <h3 className="m-0 text-[19px] font-normal leading-[25px] tracking-[-0.3px] text-[#EDF2F0]">
                        {benefit.title}
                    </h3>
                    <p className="m-0 text-sm leading-[22px] text-[#8B9B94]">{benefit.text}</p>
                </div>
            ))}
        </div>
    );
 ...
 */
function NumberedBand({ children, section }: { children: React.ReactNode; section: (typeof SECTIONS)[number] }) {
    return (
        <div
            id={section.id}
            className={cn(
                'flex w-full scroll-mt-4 flex-col gap-5 pb-11 pt-8 sm:gap-6 sm:pb-20 sm:pt-11 lg:flex-row lg:gap-14',
                GUTTER,
                BAND_RULE,
            )}
        >
            <div className="flex w-full flex-row items-center gap-3 sm:gap-3.5 lg:w-[140px] lg:flex-col lg:items-start lg:gap-1.5 xxl:w-[180px]">
                <span className="text-[30px] leading-[30px] tracking-[-1px] text-dark-accent sm:text-[38px] sm:leading-[38px] sm:tracking-[-1.2px] lg:text-[44px] lg:leading-[44px] lg:tracking-[-1.4px] xxl:text-[54px] xxl:leading-[54px] xxl:tracking-[-1.8px]">
                    {section.num}
                </span>
                <span className={cn(MONO_LABEL, 'text-[#8B9B94]')}>{section.kicker}</span>
            </div>
            <div className="flex min-w-0 flex-col gap-[26px] lg:flex-1">{children}</div>
        </div>
    );
}
...
function CodeCard({
    ...
    return (
        <div className="w-full overflow-hidden rounded-xl border border-solid border-[#242E2B] bg-[#0D1211]">
            <div
                className={cn(
    ...
    return (
        <>
            <BandIntro title="Pick your client, paste one line">
                Every snippet already points at this deployment. No key, no account, no local process to babysit.
            </BandIntro>
            <CodeCard
    ...
    return (
        <>
            <BandIntro title="Teach it to reach for the Explorer">
                Paste this into the instructions file your tool already reads, so on-chain facts stop coming from model
                memory.
            </BandIntro>
...
function Tools() {
    return (
        <>
            ...
            <ToolDoc
                name="inspect_entity"
                description="Decoded on-chain data for any address or signature — mints and token-2022 extensions, upgrade authorities, published IDLs, verified builds, instruction traces with inner instructions."
                request={
                    <>
                        <ToolParam name="identifier" requirement="required">
                            Base58, 1–128 characters: a 32-byte account address or a 64-byte transaction signature.
                        </ToolParam>
                        <ToolParam name="cluster" requirement="optional">
                            mainnet-beta · devnet · testnet · simd296. Defaults to mainnet-beta.
                        </ToolParam>
                    </>
                }
                response={
                ...
                note={
                    <p className="m-0 max-w-[820px] text-sm leading-[22px] text-[#8B9B94]">
                        Accounts owned by the legacy loaders answer with a{' '}
            ...
            <ToolDoc
                name="ping"
                description="Liveness probe — confirms the server is reachable and which cluster it answers for."
...
function ToolDoc({
    ...
    return (
        <div className={cn('flex w-full flex-col gap-3 pt-[22px]', BAND_RULE)}>
            <div className="flex w-fit items-center gap-3">
                <span className="font-mono text-base text-[#EDF2F0]">{name}</span>
                <span className="rounded-full bg-[#0D3D31] px-[7px] py-0.5 font-mono text-[10px] tracking-[0.6px] text-dark-accent">
                    read-only
                </span>
            </div>
            ...
            <div className="mt-2 w-full overflow-hidden rounded-xl border border-solid border-[#242E2B] bg-[#0D1211]">
...
function ToolParam({
    ...
    return (
        ...
        >
            <div className="flex w-full items-center gap-2.5 lg:w-[220px] lg:shrink-0">
...
function Examples() {
    ...
    // After the box has shrunk back to the picker, hold its bottom edge where it was.
    useLayoutEffect(() => {
    ...
    return (
        <>
            <div
                ref={boxRef}
                className="w-full overflow-hidden rounded-2xl border border-solid border-[#242E2B] bg-[#121716] shadow-[0px_14px_36px_-12px_#00000073]"
            >
                <div className="flex w-full items-center gap-2.5 border-0 border-b border-solid border-[#242E2B] px-3.5 py-3 sm:px-[18px] sm:py-[13px]">
                    <span className="size-[7px] rounded-full bg-dark-accent" aria-hidden />
                    <span className="font-mono text-[11.5px] text-[#8B9B94]">solana-explorer</span>
                    <span className="h-px flex-1" />
    ...
    return (
        ...
        <div className="flex min-h-[440px] w-full flex-col items-center p-4 sm:px-[26px] sm:py-6">
            ...
            <div className="flex w-full flex-1 items-center justify-center">
                <p className="m-0 max-w-[600px] text-center text-[16.5px] leading-[26px] text-[#EDF2F0] sm:text-[18px] sm:leading-[28px]">
            ...
            <div className="grid w-full max-w-[600px] grid-cols-1 gap-3 sm:grid-cols-2">
                {MCP_EXAMPLES.map(example => (
                    <button
                        ...
                        className={cn(
                            'flex cursor-pointer items-center rounded-xl border border-solid px-4 py-3 text-left text-sm leading-[22px]',
...
function ExampleAnswer({
    ...
    useEffect(() => {
        ...
        const box = boxRef.current;
        if (!box) return;
        const delta = box.getBoundingClientRect().bottom - (window.innerHeight - 16);
        ...
        <div className="flex min-h-[440px] w-full flex-col">
            <div className="mt-auto flex w-full flex-col gap-3.5 p-4 sm:p-[26px]">
                <div className="flex w-full justify-end">
                    <div
                        className={cn(
                            'max-w-[600px] rounded-[16px_16px_4px_16px] bg-[#0D3D31] px-4 py-3 text-sm leading-[22px] text-[#EDF2F0] [overflow-wrap:anywhere]',
                ...
                {answering && (
                    <>
                        ...
                        <div
                            ...
                            className="flex w-full max-w-[800px] flex-col gap-3.5 rounded-[16px_16px_16px_4px] border border-solid border-[#242E2B] bg-[#0D1211] p-3.5 sm:px-[18px] sm:py-4"
                        >
            ...
            {phase === 'done' && (
                ...
                >
                    <button
                        type="button"
                        onClick={onReset}
                        className="flex cursor-pointer items-center gap-[9px] rounded-full border border-solid border-[#242E2B] bg-[#0D1211] px-[18px] py-2.5 text-[13.5px] text-[#8B9B94] hover:text-[#EDF2F0]"
                    >
...
function ThinkingBubble() {
    return (
        <div className="flex w-fit items-center gap-1.5 rounded-[16px_16px_16px_4px] border border-solid border-[#242E2B] bg-[#0D1211] px-4 py-3.5">
...
function AnswerBlockView({ caret, revealed }: { caret: boolean; revealed: RevealedBlock }) {
    ...
    if (block.kind === 'text') {
        return (
            <p className="m-0 text-sm leading-[23px] text-[#EDF2F0]">
                {revealed.text}
                {caret && <TypingCaret />}
            </p>
        );
    ...
    return (
        <div className="w-full overflow-hidden rounded-[10px] border border-solid border-[#242E2B]">
...
function ClosingCta() {
    ...
    return (
        ...
        >
            {/* Content sits above the portaled gravity canvas (which is z-0 in this band). */}
            <div className="relative z-[1] flex w-full flex-col items-center gap-[18px]">
                ...
                <div className="flex flex-col items-center gap-2.5 pt-2.5 sm:flex-row sm:gap-3">
                    <GravityCta href="#setup" fieldRef={fieldRef} className="text-[15.5px]">
                        Set up your agent
                    </GravityCta>
                </div>
            </div>
