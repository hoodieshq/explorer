...
/**
 * Prototype toggle shown above a page while its design variants coexist for review. It floats over the
 * page on its own draggable plate — grab the handle and drop it anywhere — so it can be moved off
 * whatever it happens to be covering. Positions are relative to the nearest positioned ancestor, so the
 * plate rides the page as it scrolls; give that ancestor `relative`.
 *
 * Pairs with {@link useDesignVariant}, which keeps the selected variant in the location hash.
 */
export function DesignVariantSwitcher<T extends string>({
    label,
    variants,
    labels,
    value,
    ...
    /** Group caption on the plate, e.g. "Variant". */
    label: string;
    variants: readonly T[];
    /** Button captions; anything unlisted falls back to the raw variant id. */
    ...
    const onPointerMove = useCallback(
        (event: React.PointerEvent) => {
            ...
            if (!plate) return;
            const box = plate.getBoundingClientRect();
            // Convert the viewport pointer into the positioning parent's space by subtracting that
            // parent's own viewport offset.
            const parent = plate.offsetParent ?? document.documentElement;
