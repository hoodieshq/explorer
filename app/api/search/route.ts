...
const UTL_API_BASE = process.env.NEXT_PUBLIC_UTL_API_BASE_URL ?? 'https://token-list-api.solana.cloud';
...
async function discoverWithUtl(query: string, signal: AbortSignal): Promise<DiscoveredToken[]> {
    try {
        const url = `${UTL_API_BASE}/v1/search?query=${encodeURIComponent(query)}&chainId=101&limit=${SEARCH_TOKENS_LIMIT}`;
        const response = await fetch(url, {
...
export async function GET(request: Request) {
    ...
    try {
        const jupiterResults = await discoverWithJupiter(trimmed, discoveryController.signal);
        if (jupiterResults !== null) {
            ...
            discovered = await discoverWithUtl(trimmed, discoveryController.signal);
        }
    } finally {
    ...
    if (discovered.length === 0) {
        return NextResponse.json(
            { meta: { total: 0 }, query: trimmed, queryType, results: { tokens: [] }, success: true },
    ...
    const tokens = normalizeTokens(discovered, iconMap);
