...
- **OpenSpec — picked.** A change proposal is one self-contained markdown file (Why / What Changes / Impact). We can use it for one decision without taking on the rest of the framework.
...
- `proposal.md` is the **mandatory baseline artifact** for any captured decision. Other OpenSpec artifacts (`specs/<capability>/spec.md`, `tasks.md`, `design.md`) and lifecycle commands (`apply`, `verify`, `archive`) are **optional** — added when the change warrants them, omitted otherwise. No tooling gate requires their presence.
...
- **What changes for the next PR**: an author making a non-trivial design choice drafts a short `proposal.md` (Why / What Changes / Trade-offs considered / Impact) under `openspec/changes/<change-id>/` alongside the PR. Reviewers read it for rationale, not for spec compliance.
